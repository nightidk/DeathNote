...
public class ModEventListener {
    ...
    public static void cleanItemsTickEvent(MinecraftServer server) {
        ...
        if (tickClean == 600) sendChatMessageToAll(server.getPlayerManager().getPlayerList(), getStyledComponent("[Оповещение]", TextStyleUtil.DARK_AQUA.getStyle()).append(getStyledComponent(" 30 секунд до очистки предметов.", TextStyleUtil.WHITE.getStyle())));
        if (tickClean == 200) sendChatMessageToAll(server.getPlayerManager().getPlayerList(), getStyledComponent("[Оповещение]", TextStyleUtil.DARK_AQUA.getStyle()).append(getStyledComponent(" 10 секунд до очистки предметов.", TextStyleUtil.WHITE.getStyle())));
    ...
    public static void restartServerTickEvent(MinecraftServer server) {
        ...
        if (minutes > 0 && (minutes % 10 == 0 || minutes == 1 || minutes == 5) && tickRestart % 20 == 0) {
            sendChatMessageToAll(
                    server.getPlayerManager().getPlayerList(),
                    getStyledComponent("[Оповещение]", TextStyleUtil.DARK_AQUA.getStyle())
                            .append(getStyledComponent(" Перезагрузка сервера через " + minutes + " " + getWordInDeclension(minutes, List.of("минута", "минуты", "минут")) + ".", TextStyleUtil.WHITE.getStyle()))
            );
            return;
        }
        shouldRestart = true;
